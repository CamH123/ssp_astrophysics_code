...
def mog (ras, decs, t1, t2, t3, sunVects, sdra1, sddec1, sdra2, sddec2, sdra3, sddec3):

    #constant
    cAU = 173.144643267 * 365.25 / (2*pi)
    
    ra1 = np.random.normal(HMStoDeg(ras[0], ras[1],ras[2],True), sdra1 * pi / 180)
    ra2 = np.random.normal(HMStoDeg(ras[3], ras[4],ras[5], True), sdra2 * pi / 180)
    ra3 = np.random.normal(HMStoDeg(ras[6], ras[7],ras[8], True), sdra3 * pi / 180)
    #print(ra1, ra2, ra3)
    
    dec1 = np.random.normal(DMStoDeg(decs[0], decs[1], decs[2]) * pi / 180, sddec1 * pi / 180)
    dec2 = np.random.normal(DMStoDeg(decs[3], decs[4], decs[5])* pi / 180, sddec2 * pi / 180)
    dec3 = np.random.normal(DMStoDeg(decs[6], decs[7], decs[8])* pi / 180, sddec3 * pi / 180)
...
def histo(ras, decs, t1, t2, t3, sunVects, sdRA7, sdDec7, sdRA8, sdDec8, sdRA9, sdDec9, rep):
    a = np.array([])
    e = np.array([])
    inc = np.array([])
    lA = np.array([])
    ...
    for i in range(rep):
        r2, rd2, p = mog(ras, decs, t1, t2, t3, sunVects, sdRA7, sdDec7, sdRA8, sdDec8, sdRA9, sdDec9)
        na, ne, ni, nlongAscen, nw, nM, nT, nE, nv = od1(r2, rd2)
        a = np.append(a, na)
        e = np.append(e, ne)
        inc = np.append(inc, ni)
        
        lA = np.append(lA, nlongAscen)
        w = np.append(w, nw)
        M = np.append(M, nM)
...
sdRA9 = 0
sdDec9 = 0
sdRA8, sdDec8 = montCarlo(fieldRA, indexRA, fieldDec, indexDec, numObj)
sdRA7 = 6.316439924626232e-05
sdDec7 = 4.9975377345441117e-05
...
histo(ras, decs, dates[0], dates[1], dates[2], sunVects, sdRA7, sdDec7, sdRA8, sdDec8, sdRA9, sdDec9, 1000)

r2, rd2, p = mog(ras, decs, dates[0], dates[1], dates[2], sunVects, sdRA7, sdDec7, sdRA8, sdDec8, sdRA9, sdDec9)
